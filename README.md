# Udacity Masters in AI - Course Subtitles

Course subtitles organized by the [Udacity Master's Degree in Artificial Intelligence](https://www.udacity.com/masters-artificial-intelligence) curriculum.

## Curriculum Structure

### Core Courses (8 Required Nanodegrees)

| # | Course | Subtitles |
|---|--------|-----------|
| 1 | Introduction to Computer Programming: Part 1 | 122 files |
| 2 | Introduction to Machine Learning | - |
| 3 | Applied Data Analytics | - |
| 4 | Introduction to Deep Learning | - |
| 5 | Introduction to Artificial Intelligence | - |
| 6 | Intelligent Systems | - |
| 7 | Ethical Artificial Intelligence Practices | - |

### Electives (40+ Nanodegrees)

<details>
<summary>Click to expand full elective list</summary>

- Data Engineering with AWS
- AWS Machine Learning Engineer
- Product Manager
- AI Trading Strategies
- Artificial Intelligence
- Deep Reinforcement Learning
- Data Scientist
- Data Visualization
- Machine Learning DevOps Engineer
- AI for Healthcare
- Front End Web Developer and User Experience
- Self Driving Car Engineer
- Sensor Fusion
- Full Stack JavaScript Developer
- Digital Marketing
- Machine Learning Engineer with Microsoft Azure
- Data Science and Artificial Intelligence for Business Leaders
- Business Analytics with Tableau / Power BI
- SQL
- Backend Developer with Python
- React and Intermediate JavaScript
- Cloud DevOps Engineer
- Introduction to Programming
- Agile Software Developer
- Intermediate Python
- C++
- AWS Cloud Architect / Certified Solutions Architect
- Azure Generative AI Engineer
- Natural Language Processing
- Marketing Analytics
- Cloud Developer using Microsoft Azure
- iOS Development with SwiftUI and SwiftData
- Flying Car and Autonomous Flight Engineer
- Android Kotlin Developer
- Data Streaming
- Java Web Developer
- Agentic AI Engineer with LangChain and LangGraph
- Data Engineering with Microsoft Azure
- AI Engineer using Microsoft Azure
- AI For Business Leaders
- Computer Vision
- Data Analysis and Visualization with Microsoft Power BI
- Introduction to Cybersecurity
- Introduction to Self-Driving Cars
- Blockchain Developer

</details>

### Capstone

- Master's Degree in AI Capstone

## Directory Structure

Each core course has its own directory with subtitles organized by module:

```
core/
  01-introduction-to-computer-programming-part-1/  (122 SRT files)
    welcome-to-ai-programming-with-python/          ( 5 files)
    python-programming-fundamentals/                (18 files)
    boolean-logic-and-numeric-operators/            (15 files)
    control-flow-and-loops/                         (20 files)
    python-data-structures/                         (29 files)
    python-functions/                               (16 files)
    python-exceptions/                              (19 files)
  02-introduction-to-machine-learning/
  03-applied-data-analytics/
  04-introduction-to-deep-learning/
  05-introduction-to-artificial-intelligence/
  06-intelligent-systems/
  07-ethical-artificial-intelligence-practices/
capstone/
```

## Related Projects

| Project | Repository |
|---------|-----------|
| PCEP Capstone: Amazon Reviews Analysis | [pcep-capstone-amazon-reviews](https://github.com/LeoLin990405/pcep-capstone-amazon-reviews) |
